...
class Modes():
    ...
    def __init__(self, sim, ts, xyaver, yaver, dyn, ini):
        self.sim = sim
        self.ts = ts
        self.xyaver = xyaver
        self.yaver = yaver
    ...
    def FT(self, uz_real, norm: str):
        ...
        """
        return np.fft.fftshift(np.fft.fftn(uz_real, s=None, axes=(0, 1), norm=norm), axes=(0, 1))
    ...
    def fmodes(self, kx_tilde, qq=False):
        """
    ...
    def pmodes(self, kx_tilde, n: int):
        ...
        """
        om_sq = self.gz**2/(2*self.cs_d)**2+self.cs_d**2*((kx_tilde/self.L0)**2+((n+0.5)*np.pi/(self.L_zd))**2)
        # om_sq = self.gz**2/(2*self.cs_d)**2+self.cs_d**2*((kx_tilde/self.L0)**2+((n+0.5)*np.pi/(9*self.lz/10))**2)
        # om_sq = self.gz**2/(2*self.cs_u)**2+self.cs_u**2*((kx_tilde/self.L0)**2+((n+0.5)*np.pi/(9*self.lz/10))**2)
        return np.sqrt(om_sq/self.omega0**2)
    
    def pmodes_test(self,kx_tilde,n:int):
        # om_sq = self.gz**2/(2*self.cs_d)**2+self.cs_d**2*((kx_tilde/self.L0)**2+((n+0.5)*np.pi/self.lz)**2)
        om_sq = self.gz**2/(2*self.cs_u)**2+self.cs_u**2*((kx_tilde/self.L0)**2+((n+0.5)*np.pi/(self.L_zd))**2)
        return np.sqrt(om_sq/self.omega0**2)
