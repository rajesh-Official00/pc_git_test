...
class Modes():
    ...
    def fmodes(self, kx_tilde, qq=False):
        ...
        if qq==True:
            ...
            om_sq = self.gz*(kx_tilde/self.L0)
        freq = np.sqrt(om_sq/self.omega0**2)
    ...
    def pmodes(self, kx, n: int):
        ...
        om_sq = self.gz**2/(2*self.cs_d)**2+self.cs_d**2*((kx)**2+((n+0.5)*np.pi/(9*self.lz/10))**2)
        freq = np.sqrt(om_sq/self.omega0**2)
        return round(freq, 3)
